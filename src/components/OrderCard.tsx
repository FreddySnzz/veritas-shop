...
export default function OrderCard({ 
  ...
  const [couponInfo, setCouponInfo] = useState<CouponModel | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (order?.coupon_id && mode === "admin") {
          try {
            const coupon = await getCouponByIdAction(order.coupon_id);
            if (coupon) {
    ...
    fetchData();
  }, [order?.user_id, order?.coupon_id, mode]);

  if (loading) {
    return (
      <div className={cn("relative flex rounded-lg w-full bg-white dark:bg-input/30 p-4 gap-4", className)}>
                ...
                <div className="flex flex-col gap-6 text-sm">
                  <div className="flex gap-4 w-full">
                    ...
                    <div className="flex flex-col items-end w-full text-secondary dark:text-zinc-200">
                      ...
                      {couponInfo && (
                        <div className="flex text-xs gap-1 items-end">
                          <p>
                            Cupom aplicado:
                          </p>
                  ...
                  <div className="flex flex-col">
                    ...
                    {userInfo?.phone ? (
                      <Link 
                        ...
                        rel="noopener noreferrer"
                        target="_blank"
                        href={`https://wa.me/${userInfo?.phone}`}
                      >
