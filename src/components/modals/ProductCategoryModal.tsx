...
import { createProductCategoryAction, updateProductCategoryAction } from "@/app/actions/productsCategory.action";

interface ProductCategoryProps extends React.HTMLAttributes<HTMLElement> {
  mode: 'editar' | 'adicionar';
  initialData?: ProductCategoryModel;
  cachedCategories: ProductCategoryModel[];
  modalOpen: boolean
  onClose?: () => void
};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'editar' && initialData) {
        ...
        const result = await createProductCategoryAction(dataSubmit);
      ...
      >
        <div className="flex justify-between items-center border-b border-gray-100 dark:border-background-dark pb-4">
          <h2 className="text-xl font-bold text-gray-800 dark:text-zinc-50">
            {mode === 'editar' ? 'Editar Categoria de Produtos' : 'Nova Categoria de Produtos'}
          </h2>
          <button 
            type="button"
            aria-label="Fechar"
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col">
              <Label className="font-bold mb-1 dark:text-zinc-200">
                Imagem do Produto da Categoria (Opcional)
              </Label>
